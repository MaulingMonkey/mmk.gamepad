...
namespace mmk.gamepad {
	...
	function refreshGamepads() {
		...
		let rows = d3.select("#mmk-gamepad-metadata-1-demo").selectAll(".gamepad").data(entries);
		rows.exit().remove();
	...
	function updateRows (rows: d3.selection.Update<Entry>) {
		...
		rows.select(".gamepad-label"    ).text(gp => mmk.gamepad.metadata.getDeviceLabel(gp.display));
	...
	if ('d3' in window) addEventListener("load", function(){
		...
		if ('addEventListener' in window) {
			...
			addEventListener("mmk-gamepad-button-down",  e => eventRows.push({ type: e.type, gamepadIndex: e.gamepadIndex.toString(), index: e.buttonIndex.toString(), held: e.held ? "held" : "released", value: e.buttonValue.toFixed(2), valueLabel: metadata.getGamepadButtonLabel(e.gamepadType, e.buttonIndex) }));
			addEventListener("mmk-gamepad-button-up",    e => eventRows.push({ type: e.type, gamepadIndex: e.gamepadIndex.toString(), index: e.buttonIndex.toString(), held: e.held ? "held" : "released", value: e.buttonValue.toFixed(2), valueLabel: metadata.getGamepadButtonLabel(e.gamepadType, e.buttonIndex) }));
			addEventListener("mmk-gamepad-button-value", e => eventRows.push({ type: e.type, gamepadIndex: e.gamepadIndex.toString(), index: e.buttonIndex.toString(), held: e.held ? "held" : "released", value: e.buttonValue.toFixed(2), valueLabel: metadata.getGamepadButtonLabel(e.gamepadType, e.buttonIndex) }));
			addEventListener("mmk-gamepad-axis-value",   e => eventRows.push({ type: e.type, gamepadIndex: e.gamepadIndex.toString(), index: e.axisIndex.toString(), value: e.axisValue.toFixed(2), valueLabel: metadata.getGamepadAxisLabel(e.gamepadType, e.axisIndex) }));
		}
	});
}
